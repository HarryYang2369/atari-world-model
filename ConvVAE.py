...
class ConvVAE(object):
    ...
    def _compute_loss(self, x_t):
        ...
        diff2 = (x_t - x_recon) ** 2          # [N,1,84,84]

        # Build a weight mask: give more weight to non-background
        # e.g. pixels above some threshold
        with torch.no_grad():
            mask = (x_t > 0.02).float()      # lower threshold
            weights = 1.0 + 10.0 * mask       # 9× instead of 5×


        r_loss = torch.sum(diff2 * weights, dim=(1,2,3))
